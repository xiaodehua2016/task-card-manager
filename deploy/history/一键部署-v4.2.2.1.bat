...
REM 开始记录日志
echo ======================================== > "%LOG_FILE%"
echo    小久任务管理系统 v4.2.2.1 一键部署    >> "%LOG_FILE%"
echo ======================================== >> "%LOG_FILE%"
...
echo 执行命令: powershell -ExecutionPolicy Bypass -File "%~dp0deploy-v4.2.2.1.ps1" >> "%LOG_FILE%"
powershell -ExecutionPolicy Bypass -File "%~dp0deploy-v4.2.2.1.ps1" >> "%LOG_FILE%" 2>&1
...
echo 执行命令: call "%~dp0部署到服务器-v4.2.2.1.bat" >> "%LOG_FILE%"
call "%~dp0部署到服务器-v4.2.2.1.bat" >> "%LOG_FILE%" 2>&1
set DEPLOY_STATUS=%errorlevel%
echo 部署完成，状态码: %DEPLOY_STATUS% >> "%LOG_FILE%"
if %DEPLOY_STATUS% neq 0 (
    echo ❌ 部署过程中出现错误，请查看日志文件: %LOG_FILE%
...
echo 执行命令: call "%~dp0Git部署-v4.2.2.1.bat" >> "%LOG_FILE%"
call "%~dp0Git部署-v4.2.2.1.bat" >> "%LOG_FILE%" 2>&1
set DEPLOY_STATUS=%errorlevel%
echo 部署完成，状态码: %DEPLOY_STATUS% >> "%LOG_FILE%"
if %DEPLOY_STATUS% neq 0 (
    echo ❌ 部署过程中出现错误，请查看日志文件: %LOG_FILE%
...
echo 执行命令: powershell -ExecutionPolicy Bypass -Command "& { . '%~dp0deploy-v4.2.2.1.ps1'; Create-Package }" >> "%LOG_FILE%"
powershell -ExecutionPolicy Bypass -Command "& { . '%~dp0deploy-v4.2.2.1.ps1'; Create-Package }" >> "%LOG_FILE%" 2>&1
set DEPLOY_STATUS=%errorlevel%

# Task Manager v4.2.1 Server Deployment Script (Final Fixed Version v2.2)
# With automatic password input and enhanced error handling
...
$SERVER_USER = "root"
$PASSWORD_FILE = "C:\AA\codebuddy\1\123.txt"  # Password file with .txt extension
$REMOTE_TMP = "/tmp"
...
function Execute-SSHCommand {
    ...
    # Create temporary expect script for automatic password input
    $expectScript = @"
#!/usr/bin/expect -f
set timeout 30
...
"@
    ...
    $tempExpectFile = Join-Path -Path $env:TEMP -ChildPath "ssh_expect_$(Get-Random).exp"
    ...
    try {
        ...
        if (Get-Command "expect" -ErrorAction SilentlyContinue) {
            ...
            if (Get-Command "sshpass" -ErrorAction SilentlyContinue) {
                ...
                # Use PowerShell with environment variable
        ...
        if ($exitCode -eq 0) {
            Write-Log "${GREEN}✓ $Description completed successfully${NC}"
            return $true
        } else {
            Write-Log "${RED}✗ $Description failed with exit code: $exitCode${NC}"
            return $false
        }
    } catch {
        Write-Log "${RED}✗ $Description failed with error: $($_.Exception.Message)${NC}"
        return $false
    } finally {
...
function Execute-SCPUpload {
    ...
    try {
        ...
        if ($exitCode -eq 0) {
            Write-Log "${GREEN}✓ $Description uploaded successfully${NC}"
            return $true
        } else {
            Write-Log "${RED}✗ $Description upload failed with exit code: $exitCode${NC}"
            return $false
        }
    } catch {
        Write-Log "${RED}✗ $Description upload failed with error: $($_.Exception.Message)${NC}"
        return $false
    }
}

Write-Log "${BLUE}=== Task Manager ${VERSION} Server Deployment Script v2.2 ===${NC}"
...
Write-Log "`n${GREEN}Server deployment completed!${NC}"

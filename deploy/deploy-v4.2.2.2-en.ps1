# Task Manager System v4.2.2.2 Deployment Script
...
function Create-Package {
    ...
    if (Test-Path "data") {
        ...
        $defaultData = @{
            tasks = @{
                templates = @(
                    @{ name = "Study Task"; type = "daily" }
                    @{ name = "Work Task"; type = "daily" }
                    @{ name = "Read 30 minutes"; type = "daily" }
                    @{ name = "Exercise 30 minutes"; type = "daily" }
                    @{ name = "Writing Practice"; type = "daily" }
                )
            }
            dailyTasks = @{}
            completionHistory = @{}
    ...
    if (Test-Path "data\README.md") {
        ...
        $readmeContent = @"
# Task Manager System v4.2.2.2

## Data Directory Description

This directory contains data files required for system operation:

- shared-tasks.json: Shared task data, including task templates and completion records
- Other JSON files: System-generated data files

Please do not modify these files manually to avoid data corruption.
"@
...
# Main function
function Main {
    Write-Log "Task Manager System v4.2.2.2 Automatic Deployment Started" "INFO"

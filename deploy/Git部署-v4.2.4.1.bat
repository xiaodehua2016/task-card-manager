...
REM 开始记录日志
echo ======================================== > "%LOG_FILE%"
echo    小久任务管理系统 v4.2.4 Git部署    >> "%LOG_FILE%"
echo ======================================== >> "%LOG_FILE%"
...
git commit -m "更新任务管理系统到v4.2.4版本 - 同步功能增强和部署改进" >> "%LOG_FILE%" 2>&1
...
if %errorlevel% neq 0 (
    ...
    REM 创建README.md文件
    echo # 小久任务管理系统 v4.2.4 > README.md
    git add README.md >> "%LOG_FILE%" 2>&1
    git commit -m "📝 初始提交" >> "%LOG_FILE%" 2>&1
    
    REM 再次添加所有文件
    git add . >> "%LOG_FILE%" 2>&1
    git commit -m "更新任务管理系统到v4.2.4版本" >> "%LOG_FILE%" 2>&1
)
...
if %errorlevel% neq 0 (
    ...
    if %errorlevel% neq 0 (
        ...
        echo ✅ 代码已成功推送到远程仓库 >> "%LOG_FILE%"
        echo.
        echo ✅ Git部署完成，代码已成功推送到远程仓库
    )
)
